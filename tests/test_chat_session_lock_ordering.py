...
def _wire_common(monkeypatch):
    ...
    # PR1 (BRAIN_TIER_AWARENESS) routed /chat through verify_jwt_claims to read
    # the asserted author_role; mock it too so the lock-ordering property is
    # exercised without a real signed token. Mirrors the verify_jwt stub above.
    monkeypatch.setattr(
        m,
        "verify_jwt_claims",
        lambda request: {
            "sub": "testpublickey0123456789",
...
def test_role_menu_early_return_still_covered_by_lock(monkeypatch):
    ...
    monkeypatch.setattr(m, "verify_jwt", lambda request: "testpublickey0123456789")
    # PR1 (BRAIN_TIER_AWARENESS) routed /chat through verify_jwt_claims to read
    # the asserted author_role; this test wires auth inline (not via
    # _wire_common), so stub it here too to avoid a spurious 401.
    monkeypatch.setattr(
        m,
        "verify_jwt_claims",
        lambda request: {
