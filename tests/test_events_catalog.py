...
@pytest.mark.asyncio
@pytest.mark.xfail(reason="pre-existing: events_catalog is now a list (no .items); red all session (quarantine per governor, fix separately)")
@pytest.mark.xfail(reason="pre-existing: _refresh_events_catalog returns list, tests expect dict; quarantine for CI green, fix separately")
