...
def _redact_secrets(text: str) -> tuple[str, list[str]]:
    """Redact secret-shaped strings before public transcript publish.

    Thin lazy wrapper over ``app.redaction`` (single source of truth) so this
    stays importable as ``from app.main import _redact_secrets`` for existing
    callers and tests, while the email-watch dispatcher reuses the same rules
