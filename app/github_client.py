...
class GitHubClient:
    ...
    def _repo_has_workflows(self, pr) -> bool:
        ...
        Used as a fallback when the Checks/Status API is unreadable (e.g. a
        fine-grained PAT that lacks the Checks read permission on a private
        repo, which GitHub answers with 403). Workflow presence is visible via
        the Contents API under ordinary contents:read, so it lets us tell
        "no CI configured" (safe to warn + merge, the ``no-ci`` path) apart from
        "CI exists but we cannot read its result" (must still refuse).
        Conservative on error: return True (assume CI exists) when unsure.
        """
        try:
            repo = pr.base.repo
    ...
    def _ci_status(self, pr) -> dict:
        ...
        try:
            ...
            # lacks the Checks read permission). If the repo defines no
            # workflows at all there is genuinely no CI to gate on, so fall
            # back to the no-ci path (the caller warns and proceeds) instead of
            # blocking a merge on a token permission quirk. If workflows DO
            # exist, keep refusing so real CI is never merged unseen.
