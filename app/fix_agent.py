...
class FixAgent:
    ...
    def _build_system_prompt(self, repo: str, diagnosis: dict[str, str]) -> str:
        return (
            ...
            "4. If the fix requires creating a new file, do so.\n"
            "5. If you're stuck after 3 attempts, say 'I give up' and stop.\n"
            "6. When done, do not call any more tools.\n"
        )
    ...
    def _execute_tool(
        ...
        if func_name == "read_file":
            return self._tool_read_file(repo, args["path"], branch)
        if func_name == "edit_file":
            return self._tool_edit_file(
                repo, branch, args["path"], args["old_string"], args["new_string"]
            )
        if func_name == "grep_code":
            return self._tool_grep_code(repo, args["pattern"], args.get("path"))
        if func_name == "py_compile":
            return self._tool_py_compile(repo, branch, args["path"])
