...
composes a personalized agenda from live DAO sources (including the oracle's
advisory and QMDJ chart), and posts it in Telegram #General — so when the
governor arrives, the day's action items are already waiting.
...
def _fetch_open_prs() -> str:
    ...
    try:
        ...
        repos_to_check = [
            "truesight_autopilot", "dao_protocol", "oracle",
            "dapp_beta", "tokenomics", "go_to_market", "market_research",
            "agroverse_shop_beta", "truesight_me_beta", "capoeira",
        ]
        all_prs: list[dict] = []
...
def _compose_agenda(
    ...
    Includes the hexagram framing, DAO Advisory (DeepSeek guidance), and
    QMDJ chart metadata from the reading payload, plus live DAO context.
    """
    ...
    if reading_data:
        ...
        if primary_number and primary_name:
            oracle_sections.append(f"🌅 **Today's hexagram: {primary_number} — {primary_name}**")

        # DAO Advisory (DeepSeek-generated guidance from the oracle)
        advisory_summary = reading_data.get("advisory_summary", "")
        if advisory_summary and advisory_summary.strip():
            # Truncate to 800 chars for Telegram readability
            truncated = advisory_summary.strip()[:800]
            if len(advisory_summary) > 800:
                truncated += "…"
            oracle_sections.append(f"\n🔮 **Oracle Advisory:**\n{truncated}")

        # QMDJ chart metadata
        qmdj_card = reading_data.get("qmdj_card", "")
        if qmdj_card and qmdj_card.strip():
            oracle_sections.append(f"\n🧭 **QiMen Dunjia:** {qmdj_card.strip()[:300]}")

    oracle_block = "\n".join(oracle_sections) if oracle_sections else ""

    # ── Assemble full agenda ──
...
async def handle_daily_briefing(payload: dict, signature: str, public_key: str) -> dict:
    ...
    3. Compose agenda from live sources (including advisory + QMDJ)
    ...
    if not reading_data:
        ...
        reading_data = {
            ...
            "qmdj_card": payload.get("qmdj_card"),
            "timestamp_utc": payload.get("timestamp_utc"),
        }

    # 5. Compose agenda (includes advisory + QMDJ)
